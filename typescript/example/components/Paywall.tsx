  ...
  const activePaymentRequirements = useMemo(() => {
    ...
    const matchingPaymentMethod = paymentMethods.find(
      (method) =>
        ...
        method.tokenAddress === selectedCoin.paymentMethod?.tokenAddress
    );
    ...
    selectedPaymentMethodIndex,
    selectedNetwork.id,
    selectedCoin.paymentMethod?.tokenAddress,
  ]);
  ...
  const checkImageStatus = async (
    ...
    try {
      ...
      if (data.status === "completed" && data.filename) {
        ...
        // Still processing, retry if we have retries left
        if (retriesLeft > 0) {
    ...
    >
      ...
      {availableNetworks.length === 0 ? (
        ...
        <>
          ...
          {/* Return button */}
          {returnUrl && (
            <CancelButton onReturn={() => router.push(returnUrl)} />
          )}
        </>
      )}
    </div>
