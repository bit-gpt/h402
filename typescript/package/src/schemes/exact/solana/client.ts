...
import { safeBase64Encode } from "../../../shared/base64.js";
  ...
  const txMessage = pipe(
    ...
    (tx) => {
      try {
        ...
        console.log("Memo instruction:", logObject(memoInstruction));
...
async function sendAndCreatePayload(
  ...
  try {
    ...
    if (typeof signer.sendTransaction === "function") {
      ...
      // Wallet adapter style
      const result = await signer.sendTransaction(transaction);
      txSignature = typeof result === "string" ? result : result.toString();
      ...
      const response = await rpc.sendTransaction(signedTx).send();
      ...
      txSignature = await signer.signAndSendTransaction(transaction);
      console.log(`Transaction sent with signature: ${txSignature}`);
...
async function createPayment(
  ...
  try {
    ...
    // Use provided RPC or create a default one
    // Create a complete RPC object to satisfy type requirements
    const clusterUrl = getClusterUrl(paymentDetails.networkId);
    const fullRpc = client.rpc
      ? { ...createSolanaRpc(clusterUrl), ...client.rpc }
      : createSolanaRpc(clusterUrl);
